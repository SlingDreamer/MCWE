...
such as componets of Chinese characters or English morphemes.
...
However, the main-component "知(wisdom)" consisting of several small components is closely related to the meaning of the charactor. 
...
We denote the Chinese character set as C and the Chinese word vocabulary as W. Each character ci-C is represented by vector x_i, 
and each word wi-W is represented by vector wi.

We denote D as the training corpus, W =(w1;w2;.....;w_n) as the vocabulary of words,M_i=(m_1;m_2;...m_k) as the latent meanings in the word map for each words.
%Each word w_i(i\in{[1,n]}) has its corresponding latent meanings in the word map.
The item for wi in the word map is w_i\mapsto M_i where M_i is a collection of the latent meanings of w_i's main-components .
...
where v_{w_i} is the word embedding of w_i,\hat v_{x_{i}} is the the updated word embedding of w_i,
N_i is the number of latent meanings whose cos value between w_i is greater than 0
...
We chose two classic models including CBOW and GloVe to compare with our model.
...
In addition,we introduce a model Latent Meaning Model-Average(LMM-A),which uses the latent meanings of English morphemes to enhance the word embedddings. This model employ the morpheme embeddings to adjust the word embeddings of the target word in the training,
